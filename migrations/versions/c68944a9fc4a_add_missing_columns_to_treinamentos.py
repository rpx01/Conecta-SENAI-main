...
def upgrade() -> None:
    ...
    if "links_materiais" not in columns:
        op.add_column(
            "treinamentos", sa.Column("links_materiais", sa.JSON(), nullable=True)
        )
    if "data_criacao" not in columns:
        op.add_column(
            "treinamentos", sa.Column("data_criacao", sa.DateTime(), nullable=True)
        )
    if "data_atualizacao" not in columns:
        op.add_column(
            "treinamentos", sa.Column("data_atualizacao", sa.DateTime(), nullable=True)
        )
