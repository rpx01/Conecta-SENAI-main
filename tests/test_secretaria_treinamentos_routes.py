...
def test_crud_contatos_secretaria(client, app):
    ...
    resp = client.delete(f"/api/treinamentos/secretaria/{contato_id}", headers=headers)
    assert resp.status_code == 200
...
def test_criar_contato_email_duplicado(client, app):
    ...
    r1 = client.post("/api/treinamentos/secretaria", json=payload, headers=headers)
    assert r1.status_code == 201
    r2 = client.post("/api/treinamentos/secretaria", json=payload, headers=headers)
