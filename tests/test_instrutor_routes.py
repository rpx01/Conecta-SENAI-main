...
def test_delete_instrutor_as_non_admin_fails(client, app, non_admin_auth_headers):
    ...
    resp_del = client.delete(
        f"/api/instrutores/{instrutor_id}", headers=non_admin_auth_headers
    )
...
def test_atualizar_instrutor_email_duplicado(client, app):
    ...
    client.post(
        "/api/instrutores",
        json={"nome": "A", "email": "a@example.com"},
        headers=headers,
    )
    r2 = client.post(
        "/api/instrutores",
        json={"nome": "B", "email": "b@example.com"},
        headers=headers,
    )
    instrutor_b = r2.get_json()["id"]
    resp = client.put(
        f"/api/instrutores/{instrutor_b}",
...
def test_atualizar_instrutor_parcial(client, app):
    ...
    resp = client.put(
        f"/api/instrutores/{instrutor_id}", json={"nome": "Novo"}, headers=headers
    )
