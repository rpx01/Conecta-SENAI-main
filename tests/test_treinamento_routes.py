...
def test_criar_treinamento_nome_repetido_permitido(client, app):
    ...
    resp1 = client.post(
        ...
        json={"nome": "Treino", "codigo": "T1"},
        headers=headers,
    )
    assert resp1.status_code == 201
    resp2 = client.post(
        "/api/treinamentos/catalogo",
        json={"nome": "Treino", "codigo": "T2"},
        headers=headers,
    )
...
def test_atualizar_treinamento_nome_duplicado_permitido(client, app):
    ...
    client.post(
        "/api/treinamentos/catalogo",
        json={"nome": "A", "codigo": "C1"},
        headers=headers,
    )
    r2 = client.post(
        "/api/treinamentos/catalogo",
        json={"nome": "B", "codigo": "C2"},
        headers=headers,
    )
    tid2 = r2.get_json()["id"]
    resp = client.put(
        f"/api/treinamentos/catalogo/{tid2}", json={"nome": "A"}, headers=headers
    )
