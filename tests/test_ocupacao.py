...
def app():
    ...
    with app.app_context():
        db.create_all()
        user = User(
            nome="Test", email="test@example.com", senha="Password1!", tipo="admin"
        )
        db.session.add(user)
...
def test_excluir_ocupacao_periodo(client, app):
    ...
    resp_del = client.delete(
        f"/api/ocupacoes/{primeiro_id}", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_obter_ocupacao_periodo_completo(client, app):
    ...
    resp_get = client.get(
        f"/api/ocupacoes/{qualquer_id}", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_tendencia_ocupacoes_endpoint(client, app):
    ...
    resp = client.get(
        "/api/ocupacoes/tendencia", headers={"Authorization": f"Bearer {token}"}
    )
    assert resp.status_code == 200
    dados = resp.get_json()
    assert isinstance(dados, list)
...
def test_calendario_ocupacoes_cor_por_turno(client, app):
    ...
    resp_cal = client.get(
        "/api/ocupacoes/calendario",
        query_string={"data_inicio": hoje.isoformat(), "data_fim": hoje.isoformat()},
        headers={"Authorization": f"Bearer {token}"},
    )
