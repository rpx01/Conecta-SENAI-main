...
def test_criar_usuario(client):
    ...
    response = client.post(
        "/api/usuarios",
        json=create_payload(
            nome="Novo Usuario", email="novo@example.com", senha="Senha@123"
        ),
...
def test_login(client):
    ...
    assert all(
        "SameSite=Strict" in c
        for c in cookies
        if "access_token=" in c or "refresh_token=" in c
    )
...
def test_listar_usuarios_com_filtros(client, login_admin):
    ...
    assert all(
        "secretaria.beta" in usuario["email"] for usuario in dados_email["items"]
    )
...
def test_atualizar_senha_requer_verificacao(client):
    ...
    resp = client.post(
        "/api/usuarios",
        json=create_payload(
            nome="Teste", email="teste@example.com", senha="Original1!"
        ),
        headers={"X-CSRFToken": csrf},
...
def test_atualizar_usuario_senha_invalida(client):
    ...
    resp = client.post(
        "/api/usuarios",
        json=create_payload(
            nome="Complex", email="complex@example.com", senha="Valida1!"
        ),
        headers={"X-CSRFToken": csrf},
...
def test_non_root_admin_cannot_downgrade_admin(client, login_admin):
    ...
    headers_root = {
        "Authorization": f"Bearer {token_root}",
        "X-CSRFToken": fetch_csrf(client),
    }
    ...
    resp_promote = client.put(
        f"/api/usuarios/{novo_id}",
        json=update_payload(tipo="admin"),
    ...
    resp_downgrade = client.put(
        f"/api/usuarios/{root_id}",
        json=update_payload(tipo="comum"),
        headers=headers_new,
    )
...
def test_listar_usuarios_token_expirado(client):
    ...
    resp = client.get(
        "/api/usuarios", headers={"Authorization": f"Bearer {expired_token}"}
    )
    assert resp.status_code == 401
...
def test_non_root_admin_cannot_delete_admin(client, login_admin):
    ...
    resp_create = client.post(
        "/api/usuarios",
        json=create_payload(
            nome="Admin2", email="admin2@example.com", senha="Senha@123"
        ),
        headers={"X-CSRFToken": csrf},
    ...
    headers_admin2 = {
        "Authorization": f"Bearer {token_admin2}",
        "X-CSRFToken": fetch_csrf(client),
    }
...
def test_root_admin_can_delete_admin(client, login_admin):
    ...
    resp_create = client.post(
        "/api/usuarios",
        json=create_payload(
            nome="Admin3", email="admin3@example.com", senha="Senha@123"
        ),
        headers={"X-CSRFToken": csrf},
