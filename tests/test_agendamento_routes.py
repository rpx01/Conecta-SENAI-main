...
def test_atualizar_agendamento_data_invalida(client, login_admin):
    ...
    ag_id = r.get_json()["id"]
    resp = client.put(
        f"/api/agendamentos/{ag_id}", json={"data": "2023-02-30"}, headers=headers
    )
    assert resp.status_code == 400
...
def test_delete_agendamento_non_owner_forbidden(
    client, non_admin_auth_headers, login_admin
):
    token, _ = login_admin(client)
