...
class LancamentoRateio(db.Model):
    ...
    rateio_config_id = db.Column(
        db.Integer, db.ForeignKey("rateio_configs.id"), nullable=False
    )
    ...
    __table_args__ = (
        db.Index("ix_lancamento_instrutor_periodo", "instrutor_id", "ano", "mes"),
    )
    ...
    def to_dict(self):
        return {
            ...
            "rateio_config": (
                self.rateio_config.to_dict() if self.rateio_config else None
            ),
        }
