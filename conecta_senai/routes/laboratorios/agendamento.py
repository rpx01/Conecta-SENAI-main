...
def agendamentos_calendario(mes, ano):
    ...
    if verificar_admin(user):
        ...
        agendamentos = Agendamento.query.filter(
            Agendamento.data >= inicio_mes, Agendamento.data < fim_mes
        ).all()
    else:
...
def agendamentos_calendario_periodo():
    ...
    query = Agendamento.query.filter(
        Agendamento.data >= data_inicio, Agendamento.data <= data_fim
    )
...
def agendamentos_resumo_calendario():
    ...
    if laboratorio_filtro:
        agendamentos_query = agendamentos_query.filter(
            Agendamento.laboratorio == laboratorio_filtro
        )
    if turno_filtro:
        agendamentos_query = agendamentos_query.filter(
            Agendamento.turno == turno_filtro
        )
    ...
    return jsonify({"total_recursos": total_laboratorios, "resumo": resumo})
...
def agendamentos_visao_semanal():
    ...
    for a in agendamentos:
        ...
        turno = a.turno
        resultado.setdefault(lab, {}).setdefault(dia, {}).setdefault(turno, []).append(
            a.to_dict()
        )
...
def agenda_diaria_laboratorios():
    ...
    for turno, horarios_possiveis in HORARIOS_POR_TURNO.items():
        ...
        horarios_disponiveis = [
            h for h in horarios_possiveis if h not in horarios_ocupados
        ]
...
def verificar_disponibilidade():
    ...
    if not all([data_str, laboratorio_nome, turno]):
        return (
            jsonify(
                {"erro": "Parâmetros incompletos. Forneça data, laboratorio e turno."}
            ),
            400,
        )
    ...
    for ag in agendamentos:
        try:
            hrs = (
                ag.horarios
                if isinstance(ag.horarios, list)
                else json.loads(ag.horarios)
            )
    ...
    if formato == "pdf":
        ...
        for ag in agendamentos:
            ...
            c.drawString(
                50,
                y,
                f"{ag.id}  {nome}  {ag.data}  {ag.laboratorio}  {ag.turma}  {ag.turno}",
            )
            y -= 20
            if y < 50:
                c.showPage()
        ...
        [
            {
                ...
                "turno": l.turno,
                "data_agendamento": (
                    l.data_agendamento.isoformat() if l.data_agendamento else None
                ),
                "dados_antes": l.dados_antes,
                "dados_depois": l.dados_depois,
                "intervalo_horarios": intervalo(
                    (l.dados_depois or {}).get("horarios")
                    or (l.dados_antes or {}).get("horarios")
                ),
...
def exportar_logs_agenda():
    ...
    for l in logs:
        horarios = (l.dados_depois or {}).get("horarios") or (l.dados_antes or {}).get(
            "horarios"
        )
        writer.writerow(
            [
...
def verificar_conflitos_horarios(data, laboratorio, horarios_list, agendamento_id=None):
    return verificar_conflitos_horarios_service(
        data, laboratorio, horarios_list, agendamento_id
    )
...
def obter_kpis_dashboard():
    ...
    agendamentos_semana = Agendamento.query.filter(
        Agendamento.data >= inicio_semana, Agendamento.data <= fim_semana
    ).count()
...
def laboratorios_mais_utilizados():
    ...
    resultados = (
        db.session.query(
            Agendamento.laboratorio, func.count(Agendamento.id).label("total")
        )
        .filter(Agendamento.data >= inicio_mes, Agendamento.data <= fim_mes)
        .group_by(Agendamento.laboratorio)
    )
    resultados = resultados.order_by(func.count(Agendamento.id).desc()).all()

    return jsonify([{"laboratorio": lab, "total": total} for lab, total in resultados])
...
def tendencia_mensal_agendamentos():
    ...
    resultados = (
        ...
        .filter(extract("year", Agendamento.data) == ano)
        .group_by(extract("month", Agendamento.data))
        .order_by(extract("month", Agendamento.data))
        .all()
    )
    ...
    dados_formatados = []
    for m in range(1, 13):
        dados_formatados.append({"mes": str(m).zfill(2), "total": dados.get(m, 0)})
