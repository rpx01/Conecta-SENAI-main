...
def obter_ocupacoes_por_identificador(identificador):
    ...
    if identificador.isdigit():
        ...
        if ocupacao_base.grupo_ocupacao_id:
            ...
            ocupacoes = (
                Ocupacao.query.filter_by(grupo_ocupacao_id=grupo_id)
...
def listar_ocupacoes():
    ...
    if tipo_ocupacao:
        ocupacoes_query = ocupacoes_query.filter(
            Ocupacao.tipo_ocupacao == tipo_ocupacao
        )

    if curso_evento:
        ocupacoes_query = ocupacoes_query.filter(
            Ocupacao.curso_evento.ilike(f"%{curso_evento}%")
        )

    grupo_expr = func.coalesce(Ocupacao.grupo_ocupacao_id, cast(Ocupacao.id, String))

    agrupamento = ocupacoes_query.with_entities(
        ...
        ocupacoes_agrupadas.append(
            {
                ...
                "horario_inicio": (
                    item.horario_inicio.strftime("%H:%M")
                    if item.horario_inicio
                    else None
                ),
                "horario_fim": (
                    item.horario_fim.strftime("%H:%M") if item.horario_fim else None
                ),
                "turno": turno_nome,
                "data_inicio": (
                    item.data_inicio.isoformat() if item.data_inicio else None
                ),
...
def exportar_ocupacoes():
    ...
    if formato == "pdf":
        ...
        for oc in ocupacoes:
            ...
            c.drawString(
                50,
                y,
                f"{oc.id}  {sala}  {oc.data}  {oc.horario_inicio}  {oc.horario_fim}  {oc.status}",
            )
            y -= 20
            if y < 50:
                c.showPage()
    ...
    if formato == "xlsx":
        ...
        for oc in ocupacoes:
            sala = oc.sala.nome if oc.sala else oc.sala_id
            ws.append(
                [oc.id, sala, oc.data, oc.horario_inicio, oc.horario_fim, oc.status]
            )
        output = BytesIO()
        wb.save(output)
        output.seek(0)
...
def obter_ocupacao(identificador):
    ...
    ocupacoes_grupo, grupo_id, ocupacao_base = obter_ocupacoes_por_identificador(
        identificador
    )
    ...
    try:
        ...
        while dia <= data_fim:
            if not sala.is_disponivel(dia, horario_inicio, horario_fim):
                conflitos = Ocupacao.buscar_conflitos(
                    payload.sala_id, dia, horario_inicio, horario_fim
                )
                ...
                jsonify(
                    {
                        ...
                        "conflitos": [
                            c.to_dict(include_relations=False) for c in conflitos_totais
                        ],
                    }
                ),
                409,
        ...
        if tipo_ocupacao not in tipos_validos:
            return (
                jsonify(
                    {
                        "erro": f'Tipo de ocupação deve ser um dos seguintes: {", ".join(tipos_validos)}'
                    }
                ),
                400,
            )
        ...
        if recorrencia not in recorrencias_validas:
            return (
                jsonify(
                    {
                        "erro": f'Recorrência deve ser uma das seguintes: {", ".join(recorrencias_validas)}'
                    }
                ),
                400,
            )
...
def atualizar_ocupacao(identificador):
    ...
    ocupacoes_grupo, grupo_existente, ocupacao_original = (
        obter_ocupacoes_por_identificador(identificador)
    )
    ...
    try:
        sala_id = (
            ...
            else ocupacao_original.sala_id
        )
        ...
        turno = (
            ...
            else ocupacao_original.get_turno()
        )
        ...
        tipo_ocupacao = (
            payload.tipo_ocupacao
            if payload.tipo_ocupacao is not None
            else ocupacao_original.tipo_ocupacao
        )
        recorrencia = (
            payload.recorrencia
            if payload.recorrencia is not None
            else ocupacao_original.recorrencia
        )
        status_atual = (
            payload.status if payload.status is not None else ocupacao_original.status
        )
        observacoes = (
            payload.observacoes
            if payload.observacoes is not None
            else ocupacao_original.observacoes
        )
        curso_evento = (
            payload.curso_evento
            if payload.curso_evento is not None
            else ocupacao_original.curso_evento
        )
        ...
        if conflitos_totais:
            raise ValueError(
                "Conflito de horário detectado. A sala já está ocupada neste período."
            )

        ocupacoes_anteriores = (
            ocupacoes_grupo if grupo_id_existente else [ocupacao_original]
        )
...
def remover_ocupacao(identificador):
    ...
    ocupacoes_grupo, grupo_id, ocupacao = obter_ocupacoes_por_identificador(
        identificador
    )
    ...
    try:
        somente_dia = request.args.get(
            "somente_dia", default=False, type=lambda v: str(v).lower() == "true"
        )
        if not ocupacoes_grupo:
        ...
        return jsonify(
            {"mensagem": "Ocupação removida com sucesso", "removidas": quantidade}
        )
    except SQLAlchemyError as e:
        db.session.rollback()
        return handle_internal_error(e)
...
def verificar_disponibilidade():
    ...
    ocupacao_identificador = request.args.get("ocupacao_id")
    grupo_ocupacao_id = None
    ocupacao_id = None
    if ocupacao_identificador:
        ocupacoes_grupo, grupo_id_existente, ocupacao_base = (
            obter_ocupacoes_por_identificador(ocupacao_identificador)
        )
        if ocupacao_base:
    ...
    if not all([sala_id, data_inicio_str, data_fim_str, turno]):
        return (
            jsonify(
                {
                    "erro": "Parâmetros obrigatórios: sala_id, data_inicio, data_fim, turno"
                }
            ),
            400,
        )
    ...
    try:
        ...
        while dia <= data_fim:
            if not sala.is_disponivel(
                dia, horario_inicio, horario_fim, ocupacao_id, grupo_ocupacao_id
            ):
                disponivel = False
                conflitos.extend(
...
def obter_ocupacoes_calendario():
    ...
    if turno:
        ...
        query = query.filter(
            Ocupacao.horario_inicio == inicio, Ocupacao.horario_fim == fim
        )

    ocupacoes = query.order_by(Ocupacao.data, Ocupacao.horario_inicio).all()

    def cor_turno(t):
        cores = {"Manhã": "#FDD835", "Tarde": "#00539F", "Noite": "#512DA8"}
...
def obter_resumo_periodo():
    ...
    for dia_key, turnos in resumo.items():
        for turno, info in turnos.items():
            ...
            info["salas_livres"] = [
                nome for sid, nome in salas_dict.items() if sid not in ocupadas_ids
            ]
...
def gerar_relatorio_ocupacoes():
    ...
    total_ocupacoes = Ocupacao.query.filter(
        Ocupacao.data >= data_inicio, Ocupacao.data <= data_fim
    ).count()
    ...
    salas_mais_utilizadas = (
        db.session.query(Sala.nome, db.func.count(Ocupacao.id).label("total_ocupacoes"))
        .join(Ocupacao)
        .filter(
            Ocupacao.data >= data_inicio,
            Ocupacao.data <= data_fim,
        ...
        .group_by(Sala.id, Sala.nome)
        .order_by(db.desc("total_ocupacoes"))
        .limit(10)
        .all()
    )
    ...
    ocupacoes_por_tipo = (
        db.session.query(
            Ocupacao.tipo_ocupacao, db.func.count(Ocupacao.id).label("total")
        )
        .filter(
            Ocupacao.data >= data_inicio,
            Ocupacao.data <= data_fim,
        ...
        .group_by(Ocupacao.tipo_ocupacao)
        .all()
    )
...
def obter_tendencia_ocupacoes():
    ...
    resultados = (
        ...
        .filter(extract("year", Ocupacao.data) == ano)
        .group_by(extract("month", Ocupacao.data))
        .order_by(extract("month", Ocupacao.data))
        .all()
    )
    ...
    for i in range(1, 13):
        ...
        dados_formatados.append({"mes": mes_str, "total": dados_meses.get(mes_str, 0)})
...
def salas_utilizacao_mes():
    ...
    resultados = (
        db.session.query(Sala.nome, func.count(Ocupacao.id).label("total"))
        .join(Sala, Sala.id == Ocupacao.sala_id)
        .filter(
    ...
    return jsonify([{"sala": sala, "total": total} for sala, total in resultados])
